...
class WaiterOrderTile extends StatelessWidget {
  ...
  Color getStatusColor() {
    ...
    } else if (order.readyItems > 0) {
      return Colors.red;
    } else if (order.notReadyItems > 0) {
      return Colors.green;
    } else if (order.notReadyItems == 0 &&
        order.notAvailableItems == 0 &&
        order.totalItemsCount == order.servedItems) {
    ...
    return AppTileContainer(
      ...
      child: Column(
        ...
        children: [
          ...
          ColorCodeTile(color: getStatusColor()),
        ],
      ),
    );
  }
